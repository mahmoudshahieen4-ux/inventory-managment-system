...
- `submitSale(sale, stockUpdates)` is the **only** way to record a sale: it is `async`, generates the `id` (`crypto.randomUUID()`), `invoiceNumber` and `createdAt`, awaits `persistSaleAtomic` — invoice + line items + stock decrements in **one SQLite transaction** — and only then prepends the record. A failed write returns `null`, raises a soft toast and leaves the cart untouched so the cashier can retry. While the commit runs, `isSubmitting` is `true`, which locks the checkout button and blocks the F2 shortcut from firing a second time. Stock is applied to the UI via `useInventoryStore.applyStockDeltas` once the commit returns. (The legacy non-atomic `addSale` was removed so a checkout can never half-commit.)
- `getSaleById(id)` retrieves a stored invoice for re-printing from sales history
- **Transactions & the statement queue** (`src/services/db.ts`): the SQL plugin draws statements from a sqlx pool and acquires a connection **per statement**, so `BEGIN`/`COMMIT` only stay paired when nothing else runs in between. Every statement therefore goes through one single-flight queue, and `withTransaction` holds that queue for its whole window while running the statements of its `work` callback on the raw handle it passes in (never through the queue — that would deadlock). Nested transactional calls join the open transaction instead of issuing a second `BEGIN`, a duplicate invoice id is ignored, and a transaction left open on the connection is healed with a `ROLLBACK` retry.
